...
UPDATE `photo` AS p
INNER JOIN `zzz_property_image` AS z
ON (p.filename = z.filename)
SET p.header = z.header, p.property_id = z.property_id;

UPDATE `photo` AS p
INNER JOIN `zzz_accomodation_image` AS z
ON (p.filename = z.filename)
SET p.header = z.header, p.property_id = z.property_id;

ALTER TABLE `property` AUTO_INCREMENT=3000000;